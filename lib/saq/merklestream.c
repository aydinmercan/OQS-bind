...
SAQ__merkle_stream_recursive_build(SAQ_merkle_stream_t *ms, uint64_t start,
	...
	if (ms == NULL || out == NULL || *out != NULL || start >= end ||
	    end > ms->tree_node_count)
	{
		return SAQ_FAILURE;
	}
...
SAQ_merkle_stream_finalize_until(SAQ_merkle_stream_t *s, uint64_t *until) {
	...
	s->root = NULL;
	if (SAQ__merkle_stream_recursive_build(s, 0, s->tree_node_count,
					       &(s->root)) != SAQ_SUCCESS)
	{
		return SAQ_FAILURE;
	}
...
SAQ_merkle_stream_get_proof(SAQ_merkle_stream_t *s, uint64_t i,
	...
	if (SAQ_merkle_stream_get_root_hash_len(s, &root_hash_len) !=
	    SAQ_SUCCESS)
	{
		return SAQ_FAILURE;
	}
	...
	while (parent != NULL) {
		...
		if (left != NULL) {
			...
			if (SAQ_merkle_node_get_hash(left, left_hash,
						     &left_hash_len) !=
			    SAQ_SUCCESS)
			{
				res = SAQ_FAILURE;
			...
			if (SAQ_merkle_node_get_hash(right, right_hash,
						     &right_hash_len) !=
			    SAQ_SUCCESS)
			{
				res = SAQ_FAILURE;
	...
	hash_cleanup:
		...
		cur_node = parent;
		if (SAQ_merkle_node_get_parent(cur_node, &parent) !=
		    SAQ_SUCCESS)
		{
			return SAQ_FAILURE;
		}
...
SAQ_merkle_stream_deserialize(unsigned char *in, uint64_t in_len,
	...
	if (SAQ_merkle_stream_finalize_until(ms, &tree_node_count) !=
	    SAQ_SUCCESS)
	{
		SAQ_merkle_stream_destroy(&ms);
		return SAQ_FAILURE;
...
SAQ_merkle_stream_max_authentication_path_byte_size(SAQ_merkle_stream_t *ms,
	...
	if (SAQ_merkle_stream_get_root_hash_len(ms, &root_hash_len) !=
	    SAQ_SUCCESS)
	{
		return SAQ_FAILURE;
	}
	...
	*size = sizeof(uint32_t) +
		max_ap_len *
			(sizeof(uint8_t) + sizeof(uint64_t) + root_hash_len);
