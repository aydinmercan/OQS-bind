...
///
/// \return SAQ_SUCCESS if next node was successfully fetched; SAQ_FAILURE
/// otherwise.
...
///
/// \param[in] ap The authentication path to append the authentication node, an,
/// to.
...
/// \param[in] sibling_is_left Indicates if the sibling is a left sub-Merkle
/// tree or a right sub-Merkle tree.
/// \param[out] out Points to the created SAQ_authentication_node_t. *out must
/// be NULL.
///
...
/// \brief Fetches the current SAQ_authentication_node_t from a given
/// SAQ_authentication_path_t.
///
/// \param[in] ap The authentication path.
/// \param[out] out The fretched SAQ_authentication_node_t.
///
/// \return SAQ_SUCCESS if fetching the SAQ_authentication_node_t was
/// successful; SAQ_FAILURE otherwise.
SAQ_STATUS
...
///
/// \param[in,out] ap Points the SAQ_authentication_path_t * to be freed. *ap
/// will be set to NULL
///                on return.
///
...
/// \brief Proves whether or not a data region belongs to a given Merkle tree
/// authentication path, ap.
///
/// \param[in] data The data to be verified.
/// \param[in] data_len The length of the data.
/// \param[in] root_hash The root hash of the merkle tree that owns the
/// authentication path ap.
/// \param[in] root_hash_len The length of the root hash.
/// \param[in] ap The authentication path to be used as part of this proof.
///
/// \return SAQ_SUCCESS if data is valid given root_hash and ap; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
...
/// \brief Returns the number of bytes required to serialize a given
/// SAQ_authentication_node_t.
///
...
///
/// \return SAQ_SUCCESS if the byte size of n was computed; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
SAQ_authentication_node_byte_size(SAQ_authentication_node_t *n, uint64_t *size);

/// \brief Serializes a given SAQ_authentication_node_t and stores bytes in
/// given byte region.
///
...
///
/// \param[in] in The byte region containing the serialized
/// SAQ_authentication_node_t.
...
///
/// \return SAQ_SUCCESS if the byte size of ap was computed; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
SAQ_authentication_path_byte_size(SAQ_authentication_path_t *ap,
				  uint64_t *size);

/// \brief Serializes a given SAQ_authentication_path_t and stores bytes in
/// given byte region.
///
