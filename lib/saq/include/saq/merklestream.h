...
/// \param[out] out Points to the created SAQ_merkle_stream_t. *out must be
/// NULL.
///
...
///
/// \param[in,out] in Points the SAQ_merkle_node_t * to be freed. *in will be
/// set to NULL
///                on return.
///
...
///
/// \return SAQ_SUCCESS if data was successfully added to s; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
...
/// \brief Fetches the authentication path of the i'th block of data in the
/// Merkle tree.
///
/// \param[in] s The Merkle tree based stream.
/// \param[in] i The index of the block of data to start the authentication path
/// from.
/// \param[out] out The authentication path associated with the i'th block of
/// data. *out must be NULL.
///
/// \return SAQ_SUCCESS if an authentication path was successfully created;
/// SAQ_FAILURE otherwise.
SAQ_STATUS
...
///
/// \return SAQ_SUCCESS if the hash length was succesfully fetched; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
...
/// \param[in,out] root_hash_len Contains the length of the hash byte region.
/// *root_hash_len contains the hash length on return.
///
/// \return SAQ_SUCCESS if the hash was succesfully fetched; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
...
/// \brief Returns the number of bytes required to serialize a given Merkle tree
/// based stream.
///
...
///
/// \return SAQ_SUCCESS if the byte size of size was computed; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
SAQ_merkle_stream_byte_size(SAQ_merkle_stream_t *s, uint64_t *size);

/// \brief Serializes a given Merkle tree based stream and stores bytes in given
/// byte region.
///
...
/// \param[in,out] out_len The length of out. On return *out_len is set to the
/// number of bytes 			used to serialize n.
///
...
///
/// \param[in] in The byte region containing the serialized Merkle tree based
/// stream.
/// \param[in] in_len The length of the byte region in.
/// \param[in] hash_cb The callback to use for hashing.
/// \param[out] out The deserialized SAQ_authentication_node_t. *out must be
/// NULL.
///
/// \return SAQ_SUCCESS if in was succesfully deserialized; SAQ_FAILURE
/// otherwise.
SAQ_STATUS
...
SAQ_STATUS
SAQ_merkle_stream_max_authentication_path_byte_size(SAQ_merkle_stream_t *ms,
						    uint64_t *size);
