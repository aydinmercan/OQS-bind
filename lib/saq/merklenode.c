...
SAQ_merkle_node_new(SAQ_hash_cb_ptr hash_cb, SAQ_merkle_node_t *left,
	...
	if (data != NULL) {
		...
		if (SAQ_merkle_node_get_hash_len(left, &left_len) !=
			    SAQ_SUCCESS ||
		    SAQ_merkle_node_get_hash_len(right, &right_len) !=
			    SAQ_SUCCESS)
		{
			ret = SAQ_FAILURE;
			goto combined_cleanup;
		...
		if (SAQ_merkle_node_get_hash(left, left_hash, &left_len) !=
			    SAQ_SUCCESS ||
		...
		if (n->hash_cb(combined_hash, left_len + right_len, NULL,
			       &hash_len) != SAQ_FAILURE)
		{
			ret = SAQ_FAILURE;
			goto combined_cleanup;
		}
		...
		if (n->hash_cb(combined_hash, left_len + right_len, hash,
			       &hash_len) != SAQ_SUCCESS)
		{
			ret = SAQ_FAILURE;
		}
