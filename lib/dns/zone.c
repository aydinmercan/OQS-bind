...
trust_key(dns_zone_t *zone, dns_name_t *keyname, dns_rdata_dnskey_t *dnskey,
	...
	dns_rdata_t rdata = DNS_RDATA_INIT;
	// OQS updated from 4096 to 39000
	unsigned char data[39000], digest[ISC_MAX_MD_SIZE];
	isc_buffer_t buffer;
...
add_sigs(dns_db_t *db, dns_dbversion_t *ver, dns_name_t *name, dns_zone_t *zone,
	...
	unsigned char data[39960]; /* XXX */ // OQS increased 1024 to account
					     // for XMSSMT-SHAKE256-H60/12
					     // signatures.
	isc_buffer_t buffer;
	unsigned int i;
	bool use_kasp = false;
...
sign_a_node(dns_db_t *db, dns_zone_t *zone, dns_name_t *name,
	...
	bool offlineksk = false;
	isc_buffer_t buffer;
	unsigned char data[39960]; // OQS increased this from 1024 to account
				   // for XMSSMT-SHAKE256-H60/12 signatures
...
matchkey(dns_rdataset_t *rdset, dns_rdata_t *rr) {
	// OQS updated from 4096 to 39000
	unsigned char data1[39000], data2[39000];
